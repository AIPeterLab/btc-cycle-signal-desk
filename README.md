...
- The confirmed 50-week SMA and daily SMA50/SMA200 golden cross are early-entry overlays only.
- The 200-week SMA, BTC EMA50, BTC EMA200, realized price, estimated electrical cost per BTC, and ETF flow chart are context only. They do not override the buy-window signal.
...
- `data/etf_flows.json` - spot BTC ETF daily net flow joined with BTC daily close since ETF launch.
- `scripts/update_signals.py` - no-key updater using public BTC-USD, CoinMetrics, and ETF flow data.
...
ETF flow data is fetched from TFTC's Bitcoin ETF flow tracker, which cites Farside Investors as the source. The updater joins daily spot-BTC ETF net flow with Yahoo Finance BTC-USD daily closes and writes `data/etf_flows.json` for the dashboard chart.
...
The dashboard uses relative paths for `data/signals.json`, `data/signals.csv`, and `data/etf_flows.json`, so it works from the Cloudflare root domain without code changes.
